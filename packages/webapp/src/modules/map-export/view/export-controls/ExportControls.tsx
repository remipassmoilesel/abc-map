...
function ExportControls(props: Props) {
  ...
  const handleToggleAbcMapAttributions = useCallback(() => onAbcMapAttributionsChange(!abcMapAttributions), [abcMapAttributions, onAbcMapAttributionsChange]);

  return (
    <>
      {/* Undo redo */}
      <HistoryControls historyKey={HistoryKey.MapExport} />
      ...
      <div className={'control-block'}>
        <div className={'control-item'}>
          <button onClick={handleToggleAbcMapAttributions} disabled={!activeLayout} className={'btn btn-link'}>
            {abcMapAttributions && (
              <>
                <FaIcon icon={IconDefs.faFaceSadTear} className={'mr-2'} />
                {t('Hide_AbcMap_attributions')}
              </>
            )}
            {!abcMapAttributions && (
              <>
                <FaIcon icon={IconDefs.faSmileWink} className={'mr-2'} />
