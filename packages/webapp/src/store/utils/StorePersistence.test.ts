...
import type { PersistedMainState } from './StorePersistence.ts';
import { StorePersistence } from './StorePersistence.ts';
import { StorageKey, StorageService } from '../../core/storage/StorageService.ts';
...
import { CURRENT_STATE_VERSION } from '../state.ts';
    ...
    const sampleState: PersistedMainState = deepFreeze({
      ...
      state: {
        project: {
          ...
          lastSaveOnline: DateTime.now().toMillis(),
          lastExport: DateTime.now().toMillis(),
        },
        map: {
          currentStyle: {
            fill: {
