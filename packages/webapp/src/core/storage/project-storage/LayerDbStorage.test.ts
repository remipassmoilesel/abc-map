...
import { LayerDbStorage, logger } from './LayerDbStorage';
import { PredefinedLayerModel } from '@abc-map/shared';

logger.disable();

describe('LayerDbStorage', () => {
  let storage: LayerDbStorage;
  ...
  beforeEach(() => {
    storage = LayerDbStorage.create();
  });

  it('isStorageEnabled()', () => {
    const vectorLayer = LayerFactory.newVectorLayer();
    expect(LayerDbStorage.isStorageEnabled(vectorLayer)).toEqual(false);

    LayerDbStorage.enableVectorLayerStorage(vectorLayer);
    expect(LayerDbStorage.isStorageEnabled(vectorLayer)).toEqual(true);

    const tileLayer = LayerFactory.newPredefinedLayer(PredefinedLayerModel.OSM);
    expect(LayerDbStorage.isStorageEnabled(tileLayer)).toEqual(false);

    LayerDbStorage.enableTileLayerStorage(tileLayer);
    expect(LayerDbStorage.isStorageEnabled(tileLayer)).toEqual(true);
  });
