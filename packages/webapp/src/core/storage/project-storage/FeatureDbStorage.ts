...
import { IndexedDbClient, KvPair } from '../indexed-db/IndexedDbClient';
...
export const logger = Logger.get('FeatureDbStorage.ts');
...
export class FeatureDbStorage {
  ...
  public async putAll(layerId: string, features: GeoJSONFeature[]): Promise<void> {
    ...
    await this.client().putAll<FeatureDbStorageEntry>(ObjectStore.Features, updates);
  }
