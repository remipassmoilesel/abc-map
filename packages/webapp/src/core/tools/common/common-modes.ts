...
export const CommonConditions = {
  MoveMap: (ev: MapBrowserEvent) => ToolModeHelper.is(ev.map, CommonModes.MoveMap),
  Selection: (ev: MapBrowserEvent) => primaryAction(ev) && singleClick(ev) && noModifierKeys(ev) && ToolModeHelper.is(ev.map, CommonModes.ModifyGeometry),
  CreateGeometry: (ev: MapBrowserEvent) => primaryAction(ev) && noModifierKeys(ev) && ToolModeHelper.is(ev.map, CommonModes.CreateGeometry),
  // Modification is allowed with modification and creation mode.
  // It allows users to modify just created geometries.
  ModifyGeometry: (ev: MapBrowserEvent) => primaryAction(ev) && ToolModeHelper.is(ev.map, CommonModes.CreateGeometry, CommonModes.ModifyGeometry),
  // On linux ALT + click is often mapped, so we use shift
  DeleteVertex: (ev: MapBrowserEvent) =>
