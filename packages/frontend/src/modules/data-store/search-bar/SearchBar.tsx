...
function SearchBar(props: Props) {
  const { loading, onSearch, onFilterChange, onQueryChange, onClear, className } = props;
  ...
  return (
    <div className={clsx(Cls.searchBar, className)}>
      <input
        type={'text'}
        value={query}
      ...
      <label className="form-check-label d-flex mr-3">
        <input type={'radio'} value={ArtefactFilter.All} checked={filter === ArtefactFilter.All} onChange={handleFilterChange} className={'form-check mr-2'} />
        {t('All')}
      </label>
