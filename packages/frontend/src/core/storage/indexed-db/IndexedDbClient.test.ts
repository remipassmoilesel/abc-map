...
describe('IndexedDbClient', () => {
  ...
  it('getAll()', async () => {
    ...
    expect(await client.getAll(TestStore, ['key-3', 'key-2', 'key-1'])).toEqual([{ foo: 'bar-3' }, { foo: 'bar-2' }, { foo: 'bar-1' }]);
  });

  it('getAll() should not throw', async () => {
    expect(await client.getAll(TestStore, ['unknown-key-1'])).toEqual([]);
  });
});
