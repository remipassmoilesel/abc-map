...
export const LayoutStorage = GenericReduxDbStorage.create<AbcLayout>(DbStorage.Layouts, (st) => st.project.layouts.list);
export const SharedViewStorage = GenericReduxDbStorage.create<AbcSharedView>(DbStorage.SharedViews, (st) => st.project.sharedViews.list);
...
export class ProjectDbStorage {
  ...
  public async exists(projectId: string): Promise<boolean> {
    ...
    return client.exists(DbStorage.Projects, projectId);
  }

  public async put(project: ProjectDbStorageEntry): Promise<void> {
    const client = this.getClient();
    await client.putAll(DbStorage.Projects, [{ key: project.metadata.id, value: project }]);
  }

  public async get(projectId: string): Promise<ProjectDbStorageEntry | undefined> {
    const client = this.getClient();
    return client.get(DbStorage.Projects, projectId);
  }
