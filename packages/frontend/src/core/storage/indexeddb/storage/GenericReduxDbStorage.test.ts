...
describe('GenericReduxDbStorage', () => {
  describe('With a real storage', () => {
    ...
    beforeEach(async () => {
      store = storeFactory();
      storage = GenericReduxDbStorage.create<AbcLayout>(DbStorage.Layouts, (st) => st.project.layouts.list);
  ...
  describe('With a stubbed storage', () => {
    ...
    it('should not save if layouts does not change', async () => {
      ...
      expect(client.putAll.args).toEqual([[DbStorage.Layouts, toKvPair(layouts)]]);
    });
  });
});
