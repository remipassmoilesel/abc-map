...
export const logger = Logger.get('LayerStorage.ts');
...
export class LayerDbStorage {
  ...
  public async put(layer: LayerWrapper): Promise<void> {
    ...
    await this.client().put<LayerDbStorageEntry>(DbStorage.Layers, layerId, { layer: abcLayer });
  }

  public async getAll(ids: string[]): Promise<AbcLayer[]> {
    return this.client()
      .getAll<LayerDbStorageEntry>(DbStorage.Layers, ids)
