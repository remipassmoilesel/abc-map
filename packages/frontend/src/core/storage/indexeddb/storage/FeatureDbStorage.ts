...
import { Features_LayerIdIndex, getProjectsDbClient } from '../projects-database';
import { GeoJSONFeature } from 'ol/format/GeoJSON';
import { FeatureWrapper } from '../../../geo/features/FeatureWrapper';
import { throttleDbStorage } from './throttleDbStorage';

export const logger = Logger.get('FeatureDbStorage.ts', 'debug');
...
export class FeatureDbStorage {
  public static create(): FeatureDbStorage {
    return new FeatureDbStorage(getProjectsDbClient);
  }
  ...
  public async putAll(layerId: string, features: GeoJSONFeature[]): Promise<void> {
    const updates: KvPair<FeatureDbStorageEntry>[] = features
      .map<KvPair | null>((feature) => {
        ...
        return { key: feature.id, value: { layerId, feature } };
      })
      .filter((u): u is KvPair<FeatureDbStorageEntry> => !!u);

    await this.client().putAll<FeatureDbStorageEntry>(DbStorage.Features, updates);
  }
  ...
  public async deleteAll(layerId: string, features: GeoJSONFeature[]): Promise<void> {
    ...
    await this.client().deleteAll(DbStorage.Features, featureIds);
  }
  ...
  public getAllByLayerId(layerId: string): Promise<GeoJSONFeature[]> {
    ...
    return new Promise<GeoJSONFeature[]>((resolve, reject) => {
      this.client().openIndexCursor<FeatureDbStorageEntry>(
        DbStorage.Features,
        Features_LayerIdIndex,
        IDBKeyRange.only(layerId),
        (value, cursor) => {
          if (value) {
            result.push(value.feature);
            cursor?.continue();
          } else {
            resolve(result);
          }
