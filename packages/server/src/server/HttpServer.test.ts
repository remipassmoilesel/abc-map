...
describe('HttpServer', () => {
  describe('With a high rate limit value', () => {
    let config: Config;
    ...
    beforeEach(async () => {
      ...
      config.server.globalRateLimit.max = 200;
      config.server.globalRateLimit.timeWindow = '15min';
    ...
    describe('/api/metrics', () => {
      it('should return specific counters', async () => {
        // Prepare
        ...
        // Assert
        expect(response.body).includes('abcmap_authentications_failed 2');
        expect(response.body).includes('abcmap_anonymous_authentications_succeeded 3');
      });
    });
  });

  describe('With a low rate limit value', () => {
    let config: Config;
    let services: Services;
    let server: HttpServer;
